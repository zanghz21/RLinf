...
class Cluster:
    ...
    def _init_and_launch_managers(
        ...
        # Get node info (stored on ``_probed_*`` until :class:`NodeManager` is up;
        # :attr:`_nodes` / :attr:`_node_groups` are properties that prefer the manager).
        ...
        try:
            ...
            self._command_manager = self._launch_manager_actor(
                CommandManager, manager_node, runtime_env
            )
    ...
    def _get_node_state_from_manager(
        ...
        """Return node list, node groups, and cluster config.

        Prefers live data from :class:`NodeManager` when the proxy exists; otherwise
        falls back to the driver-side probe snapshot taken during cluster bootstrap.
        """
    ...
    def _nodes(self) -> list[NodeInfo]:
        """Node list from :class:`NodeManager` (fresh on each access)."""
        return self._get_node_state_from_manager()[0]

    @property
    def _node_groups(self) -> list[NodeGroupInfo]:
        """Node groups from :class:`NodeManager` (fresh on each access)."""
